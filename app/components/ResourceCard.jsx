...
export default class extends Component {
  ...
  deleteResource = () => {
    ...
    let dataToDelete = {}
    dataToDelete[`/goals/${goalId}/resources/${resourceId}`] = null // Set resource to null on goals object
    if (this.state.mileId) {
      dataToDelete[   // if resource also exists on a milestone, set it to null there too
        `/goals/${goalId}/milestones/${milestoneId}/resources/${resourceId}`
      ] = null
    }
      ...
      <MuiThemeProvider muiTheme={getMuiTheme(alignTheme)}>
        ...
        >
          ...
          <CardMedia style={{ padding: 15 }}>
            <img
              src={
                this.state.image ? this.state.image : "/default-placeholder.jpg"
              }
              className="resource-img"
            />
          </CardMedia>
          <CardText expandable={true}>{this.state.description}</CardText>
